       ...
       // const provider = await new common.InstancePrincipalsAuthenticationDetailsProviderBuilder().build();
        ...
        const client = new aivision.AIServiceVisionClient({ authenticationDetailsProvider: provider });
          ...
          const analyzeDocumentDetails = {
            ...
            compartmentId: "ocid1.compartment.oc1..aaaaaaaaol3rkif67etlojglvch7hymhdqtt5kqucdtgmdobbmzcc2uue64a"
          };
...
if(analyzeDocumentResponse && analyzeDocumentResponse.analyzeDocumentResult.pages[0].documentFields && analyzeDocumentResponse.analyzeDocumentResult.pages[0].documentFields.length > 0){
          console.log("Ansh length is->");
          console.log(analyzeDocumentResponse.analyzeDocumentResult.pages[0].documentFields.length);
          var array = [];
          for (let step = 0; step < analyzeDocumentResponse.analyzeDocumentResult.pages[0].documentFields.length; step++) {
            var obj = {};
console.log("Ansh Value Check-------------------->");
console.log(JSON.stringify(analyzeDocumentResponse.analyzeDocumentResult.pages[0].documentFields[step].fieldLabel.name)+":"+JSON.stringify(analyzeDocumentResponse.analyzeDocumentResult.pages[0].documentFields[step].fieldValue.value));
...
obj[analyzeDocumentResponse.analyzeDocumentResult.pages[0].documentFields[step].fieldLabel.name] = analyzeDocumentResponse.analyzeDocumentResult.pages[0].documentFields[step].fieldValue.value;
array.push(obj);
          }
...
var newarr = array.reduce(function(data,val){
